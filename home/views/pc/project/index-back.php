...
<form class="form-horizontal" action="" method="post" onsubmit="return checkform()">
		<div class="box clearfix">
				...
				<div class="contentbox">
					...
					<ul class="category clearfix">
						...
						<li><?=$i?>. <?=$d['name']?>
							...
							?>
							<li>
								<label><input type="checkbox" name="project[]" value="<?=$item['id']?>" class="project_item" data-price="<?=$item['price']?>"><?=$item['title']?></label>
								<textarea name="ab" class="hide"><?=$item['desc']?></textarea>
								<dl class="projectlcontent clearfix">
									<dt>价格:</dt>
									<dd><?=$item['price']?>元</dd>
									<dt>地点:</dt>
									<dd><?=$item['areas']?></dd>
				...
				<div class="contentbox">
					...
					<div class="box">
						<div class="items clearfix">
							...
							<div class="content"><input type="text" id="amount" name="parm[amount]" value="0" readonly="true"></div>
						</div>
						<div class="items clearfix">
							<div class="title">支付方式:</div>
							<div class="content">
								<label><input type="radio" name="parm[payment]" value="支付宝" checked="checked"> 支付宝</label>&nbsp;&nbsp;
								<label><input type="radio" name="parm[payment]" value="微信"> 微信</label>&nbsp;&nbsp;
								<label><input type="radio" name="parm[payment]" value="转账"> 转账</label>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="contentright">
				<h3>项目介绍</h3>
				<div>
				<?php $content=@$data[0]['items'][0]['desc'];
					if(empty($content)){
						$content="请选择左面的项目";
					}
					echo $content;
...
$(function(){
	...
	$(".project_item").click(function(){
		...
		var desc=$(this).parent().next("textarea").text();
		$(".contentright div").html(desc);
	})
});
