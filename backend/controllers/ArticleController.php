...
class ArticleController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $obj=DataArticle::find()->where("status != 9");
        ...
        $data=$obj->orderBy("rank desc,id desc")->offset($pages->offset)->limit($pages->limit)->all();
    ...
    /**
     * Creates a new DataArticle model.
        ...
        if(Yii::$app->request->isPost){
            ...
            if (isset($image->name)) {
                ...
                if (!file_exists(Yii::getAlias('@assetspath') . '/' . $dir)) {  
                    FileHelper::createDirectory(Yii::getAlias('@assetspath') . '/' . $dir, 777);  
                }  
    ...
    /**
     * Updates an existing DataArticle model.
    ...
    /**
     * Deletes an existing DataArticle model.
    ...
    /**
     * Finds the DataArticle model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return DataArticle the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = DataArticle::findOne($id)) !== null) {
            return $model;
        } else {
