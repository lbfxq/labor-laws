function CutImage (opt, saveCallBack) {
  this.init(opt)
...
CutImage.prototype = {
  ...
  setResizePoint: function (direction, left, top) {
    return $('<div class="resize-point" style="width:' + this._resize_point.size + 'px;height:' + this._resize_point.size + 'px;' + 'background: ' + this._resize_point.color + ';cursor:' + direction + ';position:absolute;' + 'left:' + left + 'px;top:' + top + 'px"></div>')
  },
  ...
  addUpCanvas: function () {
    ...
    if ($('#resultCanvas')[0]) {
      ...
      $result = $('<canvas width=\'' + self._img_show.width + '\' height=\'' + self._img_show.height + '\' id="resultCanvas" ></canvas>')
      $('.preview').append($result)
    }
    $result[0].getContext('2d').drawImage(self._img, self._save.left, self._save.top, self._save.width, self._save.height, 0, 0, self._img_show.width, self._img_show.height)

    self._result = $result[0].toDataURL('image/jpeg')
    self.upCanvasEvent()
  }, //绑定鼠标按下事件
  upCanvasEvent: function () {
    var self = this
    self._$canvasUp.on('mousedown', cavMouseDown)
    self._$canvasUp.on('mouseup', () => {
      self.drawCanvas()
    })
    ...
    function cavMouseDown (e) {
      ...
      var relativeOffset = {
        x: e.clientX - $(canv).offset().left,
        y: e.clientY - $(canv).offset().top,
      }
      $(document).on('mousemove', function (e) {
        ...
        $(canv).css({
          left: countPosition().left,
          top: countPosition().top,
        })//移动上层canvas
        ...
        //重绘剪裁区域
        self._ctxUp.drawImage(self._img, self._save.left, self._save.top, self._save.width, self._save.height, 0, 0, self._img_show.crop_width, self._img_show.crop_height)
  ...
  resizeEvent: function () {
    var self = this
    $('.resize-point').on('mousedown', function (e) {
      e.preventDefault()
    })
    //
    //   var pLeft = $(this).position().left + self._resize_point.size / 2,
    // pTop = $(this).position().top + self._resize_point.size / 2 var upLeft =
    // self._$canvasUp.position().left, upTop = self._$canvasUp.position().top
    // var noChangeX, noChangeY if (upLeft >= pLeft) noChangeX = -(upLeft +
    // self._img_show.crop_width)//为负在右 else noChangeX = upLeft if (upTop >=
    // pTop) noChangeY = -(upTop + self._img_show.crop_height)//为负在下 else
    // noChangeY = upTop  $(document).on('mousemove', function (e) {
    // console.log(e) if (noChangeX >= 0) { self._$canvasUp.css('left',
    // noChangeX) } else { self._$canvasUp.css('left', Math.abs(noChangeX) -
    // self._img_show.crop_width) } if (noChangeY >= 0) {
    // self._$canvasUp.css('top', noChangeY) } else {
    // self._$canvasUp.css('top', Math.abs(noChangeY) -
    // self._img_show.crop_height) } //阻止移动出图片区域 self._img_show.crop_width =
    // Math.abs(Math.abs(noChangeX) - countPosition().left)
    // self._img_show.crop_height = self._img_show.crop_width /
    // self._option.crop_scale if (noChangeX >= 0 && noChangeX +
    // self._img_show.crop_width > self._img_show.width) {
    // self._img_show.crop_width = self._img_show.width - noChangeX
    // self._img_show.crop_height = self._img_show.crop_width /
    // self._option.crop_scale } else if (noChangeX < 0 && Math.abs(noChangeX)
    // - self._img_show.crop_width < 0) { self._img_show.crop_width =
    // Math.abs(noChangeX) self._img_show.crop_height =
    // self._img_show.crop_width / self._option.crop_scale } if (noChangeY >= 0
    // && noChangeY + self._img_show.crop_height > self._img_show.height) {
    // self._img_show.crop_height = self._img_show.height - noChangeY
    // self._img_show.crop_width = self._img_show.crop_height *
    // self._option.crop_scale } else if (noChangeY < 0 && Math.abs(noChangeY)
    // - self._img_show.crop_height < 0) { self._img_show.crop_height =
    // Math.abs(noChangeY) self._img_show.crop_width =
    // self._img_show.crop_height * self._option.crop_scale } //如果宽高小于限制 if
    // (self._img_show.crop_width < self._img_show.min_width) {
    // self._img_show.crop_width = self._img_show.min_width
    // self._img_show.crop_height = self._img_show.crop_width /
    // self._option.crop_scale } if (self._img_show.crop_height <
    // self._img_show.min_height) { self._img_show.crop_height =
    // self._img_show.min_height self._img_show.crop_width =
    // self._img_show.crop_height / self._option.crop_scale }  //实际存储 if
    // (noChangeX >= 0) { self._save.left = noChangeX / self._img_show.scale }
    // else { self._save.left = (Math.abs(noChangeX) -
    // self._img_show.crop_width) / self._img_show.scale } if (noChangeY >= 0)
    // { self._save.top = noChangeY / self._img_show.scale } else {
    // self._save.top = (Math.abs(noChangeY) - self._img_show.crop_height) /
    // self._img_show.scale } self._save.width = self._img_show.crop_width /
    // self._img_show.scale self._save.height = self._img_show.crop_height /
    // self._img_show.scale  //重绘剪裁区域,修改属性宽高，否则无效 self._$canvasUp.attr('width',
    // self._img_show.crop_width) self._$canvasUp.attr('height',
    // self._img_show.crop_height) self._ctxUp.drawImage(self._img,
    // self._save.left, self._save.top, self._save.width, self._save.height, 0,
    // 0, self._img_show.crop_width, self._img_show.crop_height)
    // self.resizePosition()  function countPosition () {//鼠标在底层canvas的相对位置 var
    // left = e.clientX - self._$canvasDown.offset().left var top = e.clientY - self._$canvasDown.offset().top return { left: left, top: top, } } })  })

  },
  resizePosition: function () {
    var self = this
    self._resize_btn.$se.css({
      left: self._$canvasUp.position().left + self._img_show.crop_width - self._resize_point.size / 2,
      top: self._$canvasUp.position().top + self._img_show.crop_height - self._resize_point.size / 2,
    })//加上宽高,减去本身大小
    self._resize_btn.$sw.css({
      left: self._$canvasUp.position().left - self._resize_point.size / 2,
      top: self._$canvasUp.position().top + self._img_show.crop_height - self._resize_point.size / 2,
    })//加上宽高,减去本身大小
    self._resize_btn.$ne.css({
      left: self._$canvasUp.position().left + self._img_show.crop_width - self._resize_point.size / 2,
      top: self._$canvasUp.position().top - self._resize_point.size / 2,
    })//加上宽高,减去本身大小
    self._resize_btn.$nw.css({
      left: self._$canvasUp.position().left - self._resize_point.size / 2,
      top: self._$canvasUp.position().top - self._resize_point.size / 2,
    })//加上宽高,减去本身大小
  },
  ...
  save: function () {
    ...
    // ctx.drawImage(self._img, self._save.left, self._save.top, self._save.width, self._save.height, 0, 0, self._save.width, self._save.height)

    $result = $('<canvas width=\'' + self._option.crop_min_width + '\' height=\'' + self._option.crop_min_height + '\'></canvas>')
    $('body').append($result)
    $result[0].getContext('2d').drawImage(self._img, self._save.left, self._save.top, self._save.width, self._save.height, 0, 0, self._option.crop_min_width, self._option.crop_min_height)
    var base64Url = self._result = $result[0].toDataURL('image/jpeg')





    self._saveCallBack && self._saveCallBack(base64Url)
  ...
  drawCanvas () {
    ...
    var $result = $('#resultCanvas')
    $result[0].getContext('2d').fillRect(0, 0, self._img_show.width, self._img_show.height)
    $result[0].getContext('2d').drawImage(self._img, self._save.left, self._save.top, self._save.width, self._save.height, 0, 0, self._img_show.width, self._img_show.height)
  ...
  setShowImg: function () {
    ...
    if (this._img.width / this._img.height > this._option.crop_box_width / this._option.crop_box_height) {
      weight += 5
    } else {
      weight -= 5
    }
    if (this._img.width >= this._img.height) {weight++} else {weight--}
