...
CutImage.prototype = {
  ...
  resizeEvent: function () {
    ...
    $('.resize-point').on('mousedown', function () {
      ...
      var pLeft = $(this).position().left + self._resize_point.size / 2, pTop = $(this).position().top + self._resize_point.size / 2
      var upLeft = self._$canvasUp.position().left, upTop = self._$canvasUp.position().top
  ...
  save: function () {
    ...
    var self = this
    var $result = $('<canvas width=\'' + self._save.width + '\' height=\'' + self._save.height + '\' style="display:none"></canvas>')
    $('body').append($result)
    $result[0].getContext('2d').drawImage(self._img, self._save.left, self._save.top, self._save.width, self._save.height, 0, 0, self._save.width, self._save.height)
